...
| **Xcode** | ❌ **未装**（只有 Command Line Tools）| **Phase 1 阻塞**：XCTest 与 swift-testing 在 CLT 下均不可用（已实测确认）|

**Phase 0 完全不受阻塞，立刻可做。Phase 1 的 Task 6 起需要 Xcode。**

**用户动作（请在 Phase 0 进行的同时执行）：** 从 App Store 安装 Xcode，装完后执行
...
```bash
swift build --help > /dev/null && echo "XCTest 可用性检查：" && echo 'import XCTest' | swiftc -parse - 2>&1 | head -2
```
