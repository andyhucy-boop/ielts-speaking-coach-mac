...
- 目标 ChatGPT 应用固定为 bundle id `com.openai.chat`（ChatGPT Classic）。`com.openai.codex` 无语音能力，不是目标
...
Phase 0 的「测试」是实测观察，不是单元测试。每个任务的验收标准是：**在 `docs/phase0-findings.md` 中留下一条有明确结论的记录**（成立 / 不成立 / 部分成立 + 证据）。
...
## Task 11: VoiceEndPolicy —— 对译 voice-end-policy.mjs
