...
**只用在「浮在有颜色的面板上的浮层」**——目前两处：

- 主行动卡片上那颗「开始练习」（浮在主色面板上）
- 复盘页目标横幅上那颗「带着这条去复训」（浮在深色面板上）

> 侧边栏选中的那颗药丸曾经也用玻璃，后来撤了：**玻璃要么在、要么不在，做不出淡入淡出。**
> 而那颗药丸要跟着鼠标悬停平滑地亮起来——它是全 App 用得最多的控件，
> 十项一路划下来，硬切比玻璃重要得多。现在它是一个恒定存在的圆角矩形，只有不透明度在变。
...
动效再对，主线程卡住一样不流畅。2026-08-30 实测出的三处（都已修好，各有守卫）：

| 写法 | 代价 |
|---|---|
| `CoachTime.parse` 每次新建两个 `ISO8601DateFormatter` | 一次分配约 34–46 µs，而它在重绘热路径上一次被调几百次 |
| 把 `parse` 放进**排序比较器** | 解析次数从 n 变成「比较次数 × 2」（30 条错题实测 786 次） |
| 视图模型写成计算属性，一次 body 访问好几次 | 问题档案页一次重绘构造 4 个，实测 **485 ms** 卡在主线程 |

**规矩：** 昂贵的东西在 `body` 顶上算一次，当参数往下传。
**不要改成 `@State` 存起来**——视图模型读的是 `app.state`，存起来之后数据变了界面不更新，
那比慢得多，而且更难发现。

上百项的列表用 `LazyVStack`。**但它和 `ScrollViewReader.scrollTo` 相冲**
（滚不到还没建出来的行），所以要滚动定位的页面刻意保持非惰性。
