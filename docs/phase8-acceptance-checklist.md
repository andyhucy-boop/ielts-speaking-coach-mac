...
代码基线：分支 `phase2-bridge`，最后一条提交 `ad19997`（Task 10 + 那条源码守卫的修复）。
自动化测试现状：`swift test` **1344 条全绿**（11.888 秒，2026-08-07 实跑）。
...
### ⚠️ 一、你验的是哪一版代码（这里原来写的是「`swift test` 是红的」，**已经修好了**）

清单初稿写成的时候，Task 10（今日训练页的路线卡片改走解析器）还在工作区没提交，
`swift test` 因此红着一条。**这两件事现在都做完了**，你要验的就是下面这两条提交：

| 提交 | 是什么 |
|---|---|
| `35d849a` | Task 10：今日训练页四张路线卡片全部改走 `PracticeRouteResolver` |
| `ad19997` | 那条红掉的源码守卫：`NavigationStateTests` 比对的字面量改成 `startPractice(route,` |

红的根因不是功能坏了，**是一条 Phase 6 时期的源码守卫比对的字符串过期了**：
Task 10 给 `startPractice` 加了 `setup:` 参数（它就是「解析出来的这一场」的载体），
调用形状变成 `startPractice(route, setup: setup)`，而守卫里那句字面量没跟着改。

改法**没有放宽约束**：现在比的是 `startPractice(route,`，钉着的仍然是
「另外三条路线点下去真的会开练」，只是不再依赖参数表长什么样。
**已做突变验证**：把 `TodayView.act(_:)` 里那两处 `startPractice` 删掉再跑，
这条断言当场变红（`NavigationStateTests.swift:115`），改回后 1344 条全绿。

- [ ] 开工前自己核一遍：`git log --oneline -1` 是 `ad19997`、`git status` 是干净的、
      `swift test` 最后一行是 `Executed 1344 tests, with 0 failures`。
      **对不上就先停下报告**——否则你验的是一份说不清是哪一版的代码
...
|---|---|---|
| `swift test` 全绿 | `swift test` | ✅ **1344 条全绿**（11.888 秒）。原先那条红的已经修好并提交，来龙去脉见第 1 节第一条 |
...
> ✅ **本阶段的成败判据到这里就验完了**（8.3 一次 + 8.4 这一次，都是「重新生成之后那道题仍然打着勾」）。
> 下面 8.4.1 是**附加观察，不是判据**：那里会看到进度归零，但那不是「重新生成把进度清掉了」，
> 理由写在那一节里。**先读完再动手**，否则你会记下一个假缺陷，而且是最严重的那一级。

### 8.4.1 附加观察：把练过的题换出范围、再换回来，进度**不会**自己回来（**不是判据**）
...
- [ ] 进度显示「**已完成 0 / 8 题**」——这份 Part 2 计划里压根没有那道 Part 1 的题，是对的
- [ ] 再改回「**全真模考 + 14 天**」重新生成 →
      **进度是「已完成 0 / 43 题」，那道题前面是空心圆**（不是 1 / 43，不是实心勾）

  ⚠️ **看到 0 / 43 是当前实现的正常行为，不要记成「重新生成把进度清掉了」。** 原因是：
  「练过」这个标记**只存在计划里**（`TrainingPlan.days[].completedQuestionIds`），
  而 `PlanRegenerator.carryOverProgress` 只从**上一份计划**往新计划搬。
  上一步那份 Part 2 计划里本来就没有那道 Part 1 的题，于是搬无可搬。
  换句话说：**一道题被换出计划范围，那次「练过」就从计划里永久没了，换回来也不会回来。**
  （子代理写了临时探针照这个顺序实跑过一遍，跑完删掉了。原始打印：
  `7天Part1 生成后: 0 / 30` → `标记练过后: 1 / 30` → `14天Part1: 1 / 30 | carried: ["demo-q0"]`
  → `14天全真模考: 1 / 43 | carried: ["demo-q0"]` → `7天Part2: 0 / 8 | dropped: ["demo-q0"]`
  → `换回14天全真模考: 0 / 43 | carried: []`。）

- [ ] 请在报告第八节回答一句：**这个行为你能接受吗？**
      支持保持现状的理由：语义只有一条、一处实现，`PlanRegeneratorTests` 钉着它；
      反对的理由：那道题的练习记录明明还在「训练记录」里躺着，
      按 `sessions` 重建完成标记是做得到的，而「换出去再换回来就没了」很难跟用户解释。
      **这是产品决策，不是本次验收的通过 / 不通过项。**

- [ ] 为了让第 9 节还能验「练过的那道前面是实心勾」，把它重新标一次：
      **⌘Q 完全退出 App**，跑下面这条，再用第 6 节那条命令重开
...
for day in d['plan']['days']:
    if 'demo-q0' in day['questionIds']:
        day['completedQuestionIds']=sorted(set(day.get('completedQuestionIds') or []) | {'demo-q0'})
        hit=True
        print('已把 demo-q0 在第', day['id'], '天重新标成练过')
if not hit:
    print('没找到 demo-q0：当前计划是不是不在「全真模考 + 14 天」这一档？')
json.dump(d, open(p,'w'), ensure_ascii=False, indent=2)
"
```

> `demo-q0` 就是你在 8.2 练的那道（Part 1 + 7 天的第 1 天第一道，题干
> 「What do you like most about your home?」）；走 8.2 那条手工标记替代路径的话，
> 标的也是同一道。这一步**只是把界面恢复到 8.3 时的样子**，不影响任何判据。

- [ ] 重开后计划页进度回到「**已完成 1 / 43 题**」，那道题打着勾
...
- [ ] 卡片详情写着「计划的第 1 天，共 4 道题，已经练完 1 道。」，下面列出今天这几道题
      （现在的计划是全真模考 + 14 天：43 题分 14 天，第 1 天 4 道）
- [ ] **练过的那道前面是实心勾**，没练的是空心圆
      ← 那道就是 `demo-q0`，8.4.1 末尾重新标过的那道。**上一步没标的话这里当然全是空心圆**，
      那不是缺陷，是你跳了那一步
...
**「上次盯的目标」那一行在不在，取决于你 8.2 走的是哪条路——先对号入座，别急着记缺陷：**

| 你在 8.2 怎么做的 | 「上次」是哪一场 | 那一行 |
|---|---|---|
| **真练了一场**（主路径） | 你刚练的那一场 | **不会出现**，见下面的说明 |
| 用了 8.2 末尾那条**手工标记**命令 | 演示数据里最近的那一场 | **必须出现**：「上次盯的目标：每个回答先说完整主干句」 |

- [ ] 按上表勾一个：我走的是（真练 / 手工标记），那一行（出现了 / 没出现）

> **真练过就看不到那一行，这是对的，不是缺陷。**
> 「继续上次练习」认的「上次」是 `startedAt` 最新的那一场；你在 8.2 真练的那一场比演示数据新，
> 而它是从「按计划练今天」开出来的——`PracticeRouteResolver.resolvePlanToday` 给这一场的
> `goal` 恒为空串（计划路线不带单点目标，那是复训路线的事），存进记录里也就是空的。
> 卡片那一行只在「上次真的有目标」时才画（`TodayView` 里的 `if !last.goal.isEmpty`），
> 于是它整行不出现。**演示数据里那句「每个回答先说完整主干句」是第 6 节那段命令
> 写进最近一场的**，被你新练的这一场顶下去了。

- [ ] 顺手在报告里记一句你的判断：**计划路线开出来的练习不带目标，合理吗？**
      （现状：计划只管「今天练哪几道」，目标由复盘产出、走复训路线带入。
      不是判据，是产品观察。）
- [ ] **「上次盯的目标」出现了的话**（手工标记那条路径），点「开始练习」，
      弹层里应当出现一行「**本次目标：每个回答先说完整主干句**」
      —— 这就是「继续上次」把目标带过去了。**这一场可以真练也可以就此关掉**
- [ ] 没出现的话，这条不用验；目标有没有进提示词，用下面 9.4 那条验，而且更彻底
...
先生成一份**每天只有 1 题**的计划（10.1 刚把计划删掉，所以按钮上写的是「生成计划」），
好让「今天」只依赖一道题：
...
> ⚠️ **删掉的那道题是 `demo-q0`**（Part 1 选题保持题库原序，第 1 天第一道就是它）。
> 第 6 节说过：演示数据里**最近那一场练习**和**那个复训目标**都指着 `demo-q0`。
> 所以这一改会连带影响三张卡片，下面按实际会发生的写。**少了三张里的哪一张都不是缺陷。**

- [ ] 用第 6 节那条命令重开 App
- [ ] **今日训练页只剩「从题库自由选题」一张卡片**，另外三张**同时消失**：

| 消失的卡片 | 为什么 |
|---|---|
| 按计划练今天 | 今天安排的那道题在题库里找不到了 |
| 继续上次练习 | 「上次」练的正是 `demo-q0`（8.2 真练的那一场，或演示数据最近那一场） |
| 复训一个旧问题 | 那个复训目标出自的练习，练的也是 `demo-q0` |

三张一起消失是对的（显示一条点了没用的路线，比不显示更糟）。
**只有多出卡片才是缺陷**——显示出来却点不动，正是本阶段要防的那件事。

- [ ] 但请判断一句：**光看这一页（只剩一张卡片，没有任何解释），你看得出发生了什么吗？**
      照实写进报告——这一页确实一个字都没说，说明只在学习计划页
...
**接着照那句「在本页重新生成计划把它换掉」做一次——但它当场就会卡住，这是一条真缺陷，请照下面验：**

- [ ] 先**什么都别改**，直接看「重新生成计划」那颗按钮：**它是灰的、点不动**。
      旁边写着「Part 1（日常话题问答）现在只有 29 道题，分不满 30 天，会有整天没题可练。
      下一步：把周期改成 14 天，或先到「训练题库」页导入更多题目。」
      （表单的初值取自现有计划 = Part 1 + 30 天，而 Part 1 刚被你删掉一道，只剩 29 题）
- [ ] ⚠️ **这就是缺陷**：题目行给的下一步是「在本页重新生成计划把它换掉」，
      指过去却是一颗**按不动的按钮**；要真按下去，还得先照按钮旁边那句**另一条**说明
      把周期改掉。**两句下一步各说各的，中间那一步没人讲。**
      请写进报告第八节（铁律 6：给出的下一步必须真的管用）
- [ ] 按第二句说的，把**周期改成 14 天**（重点仍是 Part 1），按钮当场变可点，点「重新生成计划」
- [ ] 确认「题目已失效」那一行**真的没有了**，每日安排变成 14 天 29 题
...
日期：
代码基线：分支 phase2-bridge，提交 ad19997（自己跑一次 `git log --oneline -1` 核对）
...
- 练完之后进度：已完成 __ / __
- 改成 14 天重新生成之后：已完成 __ / __        ← 必须还是练过的（判据）
- 换成全真模考重新生成之后：已完成 __ / __      ← 必须还是练过的（判据）
- 换成 Part 2（题掉出范围）时那段说明的原文：
- 【附加观察，非判据】从 Part 2 再换回全真模考 + 14 天之后：已完成 __ / __
  （预期 0 / 43，理由见清单 8.4.1）。你觉得这个行为可以接受吗：
...
- 8.2 我走的是（真练 / 手工标记），「继续上次练习」卡片上「上次盯的目标」那一行（出现 / 没出现）：
  你觉得计划路线开出来的练习不带目标，合理吗：
...
- 题目从题库消失后，今日训练页/计划页分别是什么表现：
  （今日训练页预期只剩「从题库自由选题」一张，三张一起消失，见清单 10.2）
- **光看今日训练页，你看得出发生了什么吗**：
- 「题目已失效」那一行说「在本页重新生成计划把它换掉」，可按钮是灰的——你当时卡住了吗：
- 空数据目录下两页的表现：
...
## 14. 这份清单与计划 Task 11 的九处出入（子代理发现的）

写下来免得你在现场犹豫。**第 2、3、4、5、8、9 条照计划字面走会卡住或记下假缺陷。**
...
| 8 | Step 4 第 5 步「换成全真模考再重新生成，那道题仍然必须是已完成」 | 这一步**成立**；但它只对「上一份计划里还有那道题」的情形成立。**题被换出计划范围一次（例如中途换成 Part 2），那个已完成标记就从计划里永久没了，换回来也不会回来**——「练过」只存在计划里，`carryOverProgress` 只从上一份计划搬 | 8.4 是判据、**8.4.1 是附加观察**。清单初稿在这里写反了（写成「又回到已完成、进度回到 1 / 43」），实测是 **0 / 43**，已改正 |
| 9 | Step 6「把今天那道题从题库删掉 → 「按计划练今天」消失或给出说明」 | 演示数据里「上次练的」和「那个复训目标的原题」指的**都是同一道题**，所以**三张卡片同时消失**，今日训练页只剩「从题库自由选题」 | 10.2 按实际会发生的写清了，并要求你判断「光看这一页看得出发生了什么吗」 |

另外几条计划没提、但现场很容易被误判成缺陷的：

- **`swift test` 原来红着一条**，根因是 Phase 6 那条源码守卫比对的字符串没跟上 Task 10 的调用形状。
  **已经修好并提交**（`ad19997`），现在 1344 条全绿，来龙去脉见第 1 节第一条
- **真练过一场之后，「继续上次练习」卡片上不显示「上次盯的目标」**：
  「按计划练今天」开出来的那一场 `goal` 恒为空串，而它比演示数据那场新。
  **不是丢了目标**，第 9.3 节按你走的路径分成两栏写了
- **「题目已失效」那一行给的下一步指向一颗按不动的按钮**（第 10.2 节）：
  说明写「在本页重新生成计划把它换掉」，但表单初值是现有计划的 Part 1 + 30 天，
  删掉一道之后 Part 1 只剩 29 题，分不满 30 天，按钮是灰的。**这是真缺陷**，请记进报告
