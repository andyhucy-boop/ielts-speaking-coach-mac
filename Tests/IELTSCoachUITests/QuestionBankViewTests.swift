...
final class QuestionBankViewTests: XCTestCase {
    ...
    func testThePartFilterIsAControlTheUserCanActuallyOperate() throws {
        ...
        // 四个档位（0 是「全部」，1/2/3 是三个 Part）走 `QuestionPartFilter` 那一份，
        // **不许在这一页里另写一遍**：写死的话，改了档位这一页不跟。
        //
        // 它从前借的是开练弹层那份常量。多选 Part 之后那边已经不是单选分段控件了
        //（三个可以多勾的勾选框），两者管的是两件不同的事，所以各自一份出处，
        // 但每一份都只有一个出处。
        XCTAssertTrue(
            bank.contains("QuestionPartFilter.partOptions")
                && bank.contains("QuestionPartFilter.segmentTitle(forPart:"),
            "这一页的 Part 档位不是从 `QuestionPartFilter` 来的，而是自己又写了一份。"
                + "档位一旦散在两处，迟早会长成两个样子（措辞不同、档数不同），"
                + "而这不会有任何编译错误。下一步："
        ...
        XCTAssertFalse(view.contains("partSelection = 0") || view.contains("partSelection == 0"),
                       "这一页把「全部」写成了字面的 0。下一步：用 `QuestionPartFilter.allParts`——"
                           + "「全部」这一档只能有一个出处。")
    }
