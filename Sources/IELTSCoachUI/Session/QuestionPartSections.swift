...
///
/// ## 它和那排「这一场练哪个 Part」的分段控件是什么关系
///
/// **不是两套筛选，是一件事的两层**，所以不会打架：
///
/// - 分段控件（`PracticePicker.partOptions`，自由选题弹层与训练题库页共用同一份档位
///   与同一份措辞）决定**这一场按哪个 Part 的考法跑**——它还挂着那颗
///   「练完 Part 2 接着练 Part 3」，选出来的档位会一路走进 `SessionSetup`；
/// - 这里的分栏只决定**列表怎么排、先看哪一堆**，一道题都不筛掉，也不影响这一场的考法。
///
/// 关键在于：分段控件停在「全部」那一档时它什么都没筛掉，而那一档正是用户要滑过
/// 159 条的那一档——分栏补的就是分段控件补不了的那一档。单 Part 档下同样画栏标题
/// （只有一栏、默认展开），所以两种档位下的交互完全一样，不会出现
/// 「全部档一个样、单 Part 档另一个样」。
