...
public enum RetrainingStep: String, CaseIterable, Identifiable, Equatable, Sendable {
    ...
    /// 这一步要不要给一个**看得见的答题骨架**（只有「重答原题」那一步给）。
    ///
    /// 上游的复训是四级阶梯，第 2 级就是 Scaffold：`claim → because →
    /// specific example → result`。本项目的三步大致对应它的后两级，第 1、2 级没有——
    /// 于是「重答原题」那一屏上只有题目和目标，没有任何可以往里填的结构。
    ///
    /// **它和「照着高分版念一遍不叫复训」不冲突**，两者差别是实的：
    /// 高分版是**这道题的答案**，念一遍等于背；骨架是**一个空的形状**，
    /// 四个词里没有一个字属于这道题，填什么全靠他自己。
    /// 上游那三档提示（完整英文 → 关键词+中文 → 只有中文）本项目已经实测否决过一次，
    /// 因为那三档给的都是内容；这一条给的不是内容。
    public var showsAnswerFrame: Bool { self == .rehearsal }

    /// 那个骨架本身。**四个词，不给例子**——给了例子就又变成内容了。
    ///
    /// 用户自己点名的毛病正是「内容比较空洞」，而这四步恰好就是治空洞的：
    /// 空洞的回答几乎总是停在第一步（给了观点就没了）。
    public static let answerFrame = "观点 → 因为… → 一个具体例子 → 所以…"
