...
public enum QuestionBankImport {
    /// 文件面板放行的扩展名。**面板与 `parse` 必须用同一份清单**——
    /// 面板放行了这里却不认，用户会选完文件才被拒，白跑一趟。
    public static let supportedExtensions = ["csv", "json"]

    /// 按扩展名选导入器。
    ///
    /// **不认识的扩展名必须报错，不能默默当成 CSV。**
    /// 命令行那份实现（`Sources/coach/QuestionsCommand.swift`）走的是「不是 .json 就当 CSV」，
    /// 在终端里尚可（路径是用户自己敲的）；放到界面上就不行——Task 8 要把 PDF 加进文件面板，
    /// 一份 PDF 被当成 CSV 解析，会把二进制垃圾当成题目灌进题库，而且一声不吭（铁律 7）。
    public static func parse(fileName: String, text: String) throws -> ImportResult {
        ...
        switch ext {
        case "csv": return try QuestionBankImporter.importCSV(text, sourceTitle: title)
        case "json": return try QuestionBankImporter.importJSON(text, sourceTitle: title)
        default:
            // 没有扩展名时不能拼成「扩展名是「.」」——那句话读起来像程序自己出了错。
            let what = ext.isEmpty ? "没有扩展名" : "扩展名是「.\(ext)」"
            throw CoachError.questionBankInvalid(
                "「\(fileName)」\(what)，本页只认 .csv 和 .json 两种题库文件。"
